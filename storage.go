...
func (s *FileStorage) SaveNotesFile(fileData NotesFile) error {
	outData, err := json.MarshalIndent(fileData, "", "  ")
	if err != nil {
		return err
	}
...
func (s *FileStorage) LoadNotesFile() (NotesFile, error) {
	var fileData NotesFile
	fileData.Courses = make(map[string][]NoteEntry)
	...
	if err == nil {
		err = json.Unmarshal(data, &fileData)
		if err != nil {
			return fileData, fmt.Errorf("failed to parse existing %s: %w", s.FilePath, err)
		}
	} else if !os.IsNotExist(err) {
		return fileData, err
	}
